...
import { championPortraitPath } from "@/lib/champion-image";
  ...
  if (!data.some((r) => r[metric] !== null))
    return (
      <div className="chart-empty">
        <Crosshair size={30} />
        <strong>这些对局暂未提供{metric === "teamElo" ? " Team ELO" : "评分"}</strong>
        <p>仍可查看胜负和逐场战绩，缺失值不参与计算。</p>
      </div>
    );
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={data} margin={{ top: 20, right: 14, bottom: 0, left: -12 }}>
        <defs>
        ...
        <CartesianGrid vertical={false} stroke="#263241" strokeDasharray="3 5" />
        <XAxis
          dataKey="index"
        ...
        />
        <YAxis
          domain={metric === "teamElo" ? ["dataMin - 150", "dataMax + 150"] : [0, "auto"]}
          tick={{ fill: "#96a6b8", fontSize: 12 }}
          axisLine={false}
          tickLine={false}
        />
          ...
          content={({ active, payload }) => {
            ...
            return active && r ? (
              <div className="chart-tooltip">
                ...
                <p>
                  {r.date} · {r.win === true ? "胜利" : r.win === false ? "失败" : "未判定"}
                </p>
                <span>
