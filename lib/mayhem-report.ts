...
export function mayhemReport(snapshot: Snapshot) {
  ...
  const complete =
    ...
    rows.length > 0 &&
    rows.every(
      (r) => r.detailState !== 'pending' && r.detailState !== 'unavailable',
    );
  ...
  if (
    full &&
    rates.participation.count === rows.length &&
    rates.participation.value! >= 70
  ) {
    title = '团战常驻选手';
    ...
    full &&
    rates.dpm.count === rows.length &&
    rates.dpm.value! >= 2500
  ) {
    title = '火力担当';
  ...
  const best = (key: 'damage' | 'score' | 'participation') =>
    rows
      ...
      .reduce<Match | null>(
        (winner, r) => (!winner || r[key]! > winner[key]! ? r : winner),
        null,
      );
...
export function reportText(
  ...
  const name = hideName ? '神秘海斗玩家' : snapshot.player;
