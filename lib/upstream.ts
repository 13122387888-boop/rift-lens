...
export function normalizeMatch(m: Raw, detail: Raw, openId: string, name: string): Match {
  ...
  const teams = p.teamId !== undefined ? ts.filter((t) => plain(t.teamId) === plain(p.teamId)) : [];
  const other = p.teamId !== undefined ? ts.filter((t) => plain(t.teamId) !== plain(p.teamId)) : [];
  const kda = /^(\d+)\/(\d+)\/(\d+)$/.exec(plain(p.scoreInfo));
  const duration = /用时(\d+)分(\d+)秒/.exec(plain(m.title));
  const date = /\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}/.exec(plain(m.titleTime))?.[0] ?? "日期未提供";
  const id = plain(m.championId),
    e = obj(p.echartsMap);
  const ownTeam = ps.filter((t) => plain(t.teamId) === plain(p.teamId));
  const rosterComplete =
    found.length === 1 &&
    ["100", "200"].includes(plain(p.teamId)) &&
    ps.length === 10 &&
    ownTeam.length === 5 &&
    ps.every(
      (t) => ["100", "200"].includes(plain(t.teamId)) && plain(t.openIdNow) && plain(t.nickNameStr),
    ) &&
    new Set(ps.map((t) => plain(t.openIdNow))).size === 10;
  const teammates = rosterComplete
    ? ownTeam
        .filter((t) => t !== p)
        .map((t) => ({
          // Keep the source account identifier out of the public sample and UI.
          id: md5(
            (plain(t.translateAreaId) || plain(p.translateAreaId)) + "|" + plain(t.openIdNow),
          ),
          name: plain(t.nickNameStr),
          championId: plain(t.detailChampionId),
        }))
    : undefined;
  return {
    ...(teammates ? { teammates, teammatesFetchedAt: new Date().toISOString() } : {}),
    id: plain(m.gameId),
    date,
    ...
    participation:
      num(e.killAssisScore) !== null && num(e.killAssisScore)! >= 0 && num(e.killAssisScore)! <= 100
        ? num(e.killAssisScore)
        : null,
    position: plain(p.position),
    mvp: m.wasMvp === 1 || m.wasMvp === "1" || m.wasMvp === true || p.wasMvp === "1",
    svp: m.wasSvp === 1 || m.wasSvp === "1" || m.wasSvp === true || p.wasSvp === "1",
...
async function runQuery(q: Query, signal: AbortSignal, stop: (reason: unknown) => void, emit?: (event: QueryEvent) => void): Promise<Snapshot> {
  ...
  const areaId = AREAS[q.area],
    warnings: string[] = [];
  async function get(path: string, params: Record<string, string | number>): Promise<Raw> {
  ...
  if (
    ...
    raw.data.some((m: unknown) => typeof obj(m).gameId !== "string" || !plain(obj(m).gameId))
  )
  ...
  if (q.mode === "mayhem")
    ...
    warnings.push("请求 " + q.count + " 场，数据源实际返回 " + matches.length + " 场。");
  ...
  async function loadSummary() {
    try {
      ...
      for (const field of [
        ...
        const match = /([^：:<>]+)[：:]\s*(\d+(?:\.\d+)?)/.exec(plain(summary[field]));
  ...
  if (!failed && !summaryFailed)
    queryCache.set(key, result, Math.max(0, 60_000 - (Date.now() - Date.parse(result.fetchedAt))));
  emit?.({ type: "complete", data: result });
  return result;
}
